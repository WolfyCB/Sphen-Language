...
void init_keywords(){
    ...
    hashToken(keywords, "true", 4, VALUE, BOOL_TRUE);
    ...
    hashToken(keywords, "false", 5, VALUE, BOOL_FALSE);
...
static inline TokenBucket* hashId(TokenBucket* tbl, const char* s, size_t len){
    return &tbl[_HASH_ID(s, len)];
}

Token getKeywordToken(const char* str, size_t len){
...
Token getPunctToken(const char* str, size_t len){
...
Token getOperatorToken(const char* str, size_t len){
